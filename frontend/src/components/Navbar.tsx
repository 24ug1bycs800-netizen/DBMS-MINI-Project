...
import { Film, User, LogOut, ChevronDown, LayoutDashboard, Ticket, Moon, Menu, X, Bell } from "lucide-react";
import api from "../services/api.js";

const G = "#C9A84C";

interface Notification { id: number; message: string; isRead: boolean; createdAt: string; }
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => { setMobileOpen(false); setMenuOpen(false); setNotifOpen(false); }, [pathname]);
  ...
  return (
    <>
      <style>{`
        ...
        .cc-notif-panel { position:absolute; right:0; top:calc(100% + 10px); width:320px; background:rgba(8,8,8,0.97); border:1px solid rgba(201,168,76,0.15); border-radius:12px; box-shadow:0 20px 60px rgba(0,0,0,0.85); backdrop-filter:blur(20px); overflow:hidden; animation:dropIn 0.18s ease both; }
        ...
        .cc-drop-email { font-size:0.72rem; color:rgba(255,255,255,0.3); margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        ...
        .cc-bell-btn { position:relative; background:none; border:none; cursor:pointer; color:rgba(255,255,255,0.5); padding:5px; border-radius:7px; display:flex; align-items:center; transition:color 0.2s; }
        .cc-bell-btn:hover { color:#C9A84C; }
        .cc-badge { position:absolute; top:-3px; right:-3px; min-width:16px; height:16px; background:#ef4444; border-radius:999px; font-size:9px; font-weight:900; color:#fff; display:flex; align-items:center; justify-content:center; padding:0 3px; border:2px solid #080808; }
        .cc-notif-item { padding:10px 14px; border-bottom:1px solid rgba(255,255,255,0.04); font-size:0.78rem; line-height:1.5; color:rgba(255,255,255,0.6); transition:background 0.15s; }
        .cc-notif-item:hover { background:rgba(255,255,255,0.03); }
        .cc-notif-item.unread { color:#fff; background:rgba(201,168,76,0.03); }
        .cc-notif-dot { width:5px; height:5px; border-radius:50%; background:#C9A84C; flex-shrink:0; margin-top:5px; }
      `}</style>
          ...
          <div className="cc-right">
            {user && (
              <div ref={notifRef} style={{ position: "relative" }}>
                <button className="cc-bell-btn" onClick={handleOpenNotifs} title="Notifications">
                  <Bell size={18} />
                  {unreadCount > 0 && (
                ...
                {notifOpen && (
                  <div className="cc-notif-panel">
                    <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "#fff" }}>Notifications</span>
                      {notifications.length > 0 && (
                        <span style={{ fontSize: "0.68rem", color: "rgba(201,168,76,0.7)" }}>{notifications.length} total</span>
                      )}
                    </div>
                    <div style={{ maxHeight: 320, overflowY: "auto" }}>
                      {notifications.length === 0 ? (
                        <div style={{ padding: "24px 14px", textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: "0.8rem" }}>
                          No notifications yet
                        </div>
                      ) : notifications.map(n => (
                        <div key={n.id} className={`cc-notif-item${n.isRead ? "" : " unread"}`} style={{ display: "flex", gap: 8 }}>
                          {!n.isRead && <div className="cc-notif-dot" />}
            ...
            {user ? (
              <div ref={dropRef} style={{ position: "relative" }}>
                <button className="cc-user-btn" onClick={() => { setMenuOpen(!menuOpen); setNotifOpen(false); }}>
